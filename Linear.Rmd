...
#vde
attach(final_merged_vde)
fit_vde_whole <- lm(coredata.volvde. ~ . - date, data = final_merged_vde)
summary(fit_vde_whole)
...
fit_vde_adjusted <- lm(coredata.volvde. ~ intl_job_claims_value + chifed_nfci_detrended + ur_detr + cpi_detr, data = final_merged_vde)

fit_vde_daily <- lm(coredata.volvde. ~ detr_crude_oil_prices + detr_intl_dol + detr_yield)

fit_vde_weekly <- lm(coredata.volvde. ~ intl_job_claims_value + chifed_nfci_detrended + fed_bal_detrended + treas_gen_acct_detrended, data = final_merged_vde)

fit_vde_monthly <- lm(coredata.volvde. ~ m.1_detrended + ur_detr + cpi_detr)


#define list of models
models <- list(fit_vde_whole, fit_vde_adjusted, fit_vde_daily, fit_vde_weekly, fit_vde_monthly)

#specify model names
mod.names <- c('whole', 'adjusted', 'daily', 'weekly', 'monthly')
...
```{r}
#vht
fit_vht_whole <- lm(coredata.volvht. ~ . - date, data = final_merged_vht)
summary(fit_vht_whole)
...
fit_vht_adjusted <- lm(coredata.volvht. ~ intl_job_claims_value + chifed_nfci_detrended + fed_bal_detrended + cpi_detr, data = final_merged_vht)

fit_vht_daily <- lm(coredata.volvht. ~ detr_crude_oil_prices + detr_intl_dol + detr_yield, data = final_merged_vht)

fit_vht_weekly <- lm(coredata.volvht. ~ intl_job_claims_value + chifed_nfci_detrended + fed_bal_detrended + treas_gen_acct_detrended, data = final_merged_vht)

fit_vht_monthly <- lm(coredata.volvht. ~ m.1_detrended + ur_detr + cpi_detr, data = final_merged_vht)


#define list of models
models <- list(fit_vht_whole, fit_vht_adjusted, fit_vht_daily, fit_vht_weekly, fit_vht_monthly)

#specify model names
mod.names <- c('whole_vht', 'adjusted_vht', 'daily_vht', 'weekly_vht', 'monthly_vht')
