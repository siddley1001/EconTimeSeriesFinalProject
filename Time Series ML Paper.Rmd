...
```
```{r}
volvde_df <- data.frame(date = as.Date(index(volvde)), 
                    coredata(volvde))
final_merged_vde <- merge(final_indicator_df, volvde_df, by = "date")
volvht_df <- data.frame(date = as.Date(index(volvht)), 
                    coredata(volvht))
final_merged_vht <- merge(final_indicator_df, volvht_df, by = "date")
