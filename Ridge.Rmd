...
```{r} 
##Creating ridge and pulling coefficients for VDE 
...
while(new_lambda < 0.01){
  ...
  fit_AIC <- glmnet(x_vde, y_vde, alpha = 0, lambda = new_lambda)
...
good_RIDGE <- glmnet(x_vde, y_vde, alpha = 0, lambda = lamb_min)
coef(good_RIDGE)
print(AIC_min)
print(lamb_min)
plot(AIC_vals, lamb_vals)

y_predicted <- predict(good_RIDGE, s = lamb_min, newx = x_vde)
...
##Creating ridge and pulling coefficients for  VHT
...
while(new_lambda < 0.01){
  lamb_vals <- c(lamb_vals, new_lambda)
  fit_AIC <- glmnet(x_vht, y_vht, alpha = 0, lambda = new_lambda)
  tLL <- - deviance(fit_AIC)
  k <- fit_AIC$df
  n <- fit_AIC$nobs
  AICc <- -tLL+2*k
...
good_RIDGE <- glmnet(x_vht, y_vht, alpha = 0, lambda = lamb_min)
coef(good_RIDGE)
print(AIC_min)
print(lamb_min)
plot(AIC_vals, lamb_vals)

y_predicted <- predict(good_RIDGE, s = lamb_min, newx = x_vht)
...
``{r} 
require(glmnet)
ridge.vde<- cv.glmnet(vde_train, vde.train$coredata.volvde., alpha = 0 )
ridge.vde.lambda <-ridge.vde$lambda.min
ridge.vde.lambda
plot(ridge.vde)
opt_RIDGE_model <- glmnet(x_vde, y_vde, alpha = 0, lambda = ridge.vde.lambda)
ridge.vht<- cv.glmnet(vht_train, vht.train$coredata.volvht., alpha = 0 )
ridge.vht.lambda <-ridge.vht$lambda.min
ridge.vht.lambda
plot(ridge.vht)
```
